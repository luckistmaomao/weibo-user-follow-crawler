...
def get_prelogin_status(username):
    ...
    try:
        ...
        scheduler_logger.debug('Getting prelogin status met error!')
        return None
...
def login(username, pwd, cookie_file):
    ...
    if os.path.exists(cookie_file):
        try:
            ...
            loaded = 0
            scheduler_logger.debug('Loading cookies failed!')
...
#     print p.search(text).group(1)
    try:
        ...
        scheduler_logger.debug(login_url)
        ...
        scheduler_logger.debug('redirect failed!')
        return 0
...
if __name__ == '__main__':
    print login('chengchuan90@126.com','900119','./weibo_login_cookies.dat')
